...
func TestAdminRoleCRUDLifecycleOverHTTP(t *testing.T) {
	...
	decodeData(t, decodeEnvelope(t, rec), &created)
	require.NotZero(t, created.ID)
	require.Equal(t, "Support", created.Name)
	require.Equal(t, []string{permissions.LogRead.String()}, created.Permissions)
	require.Equal(t, []string{permissions.LogRead.String()}, app.casbinClient.GetRolePermissions(created.ID))
	...
	require.Equal(t, []string{permissions.UserRead.String()}, app.casbinClient.GetRolePermissions(created.ID))
	...
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	require.Empty(t, app.casbinClient.GetRolePermissions(created.ID), "casbin grants must be cleaned up on delete")
